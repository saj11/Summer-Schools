...
//////////////////////////////////////////////////////////////////////////////////////////
// miniWeather
// Author Upgrades: Jorge Castro, CENAT
//                 Francisco Arroyo, UCR
//                 Malcolm Davies, TEC
//                 Joseph Salazar, TEC
//
//////////////////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char **argv) {
  ...
  while (etime < sim_time) {
    ...
    #pragma acc update host(state[:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) async
    //Inform the user
...
void semi_discrete_step( double *state_init , double *state_forcing , double *state_out , double dt , int dir , double *flux , double *tend ) {
  ...
  #pragma acc data present(state_init[:(nx+2*hs)*(nz+2*hs)*NUM_VARS], tend[:nx*nz*NUM_VARS], state_out[:(nx+2*hs)*(nz+2*hs)*NUM_VARS])
...
void compute_tendencies_x( double *state , double *flux , double *tend ) {
  ...
  #pragma acc data present( hy_dens_cell[2:nz], hy_dens_theta_cell[2:nz], flux[:(nx+1)*(nz+1)*NUM_VARS], state[:(nx+2*hs)*(nz+2*hs)*NUM_VARS] )
  #pragma acc parallel loop collapse(2) private(inds,stencil, vals, d3_vals, r, u, w, t, p)
  for (k=0; k<nz; k++) {
    //#pragma acc data present(flux)
    for (i=0; i<nx+1; i++) {
  ...
  #pragma acc data present(flux[:(nx+1)*(nz+1)*NUM_VARS], tend[:nx*nz*NUM_VARS])
...
void compute_tendencies_z( double *state , double *flux , double *tend ) {
  ...
  /////////////////////////////////////////////////
  //Compute fluxes in the x-direction for each cell
  #pragma acc data present( hy_dens_int[:nz+1],  hy_dens_theta_int[:nz+1], hy_pressure_int[:nz+1], flux[:(nx+1)*(nz+1)*NUM_VARS], state[:(nx+2*hs)*(nz+2*hs)*NUM_VARS])
  #pragma acc parallel loop collapse(2) private(inds, stencil, vals, d3_vals,  r, u, w, t, p) 
  for (k=0; k<nz+1; k++) {
    for (i=0; i<nx; i++) {
      //Use fourth-order interpolation from four cell averages to compute the value at the interface in question
  ...
  /////////////////////////////////////////////////
  //Use the fluxes to compute tendencies for each cell
  #pragma acc data present(state[:(nx+2*hs)*(nz+2*hs)*NUM_VARS], flux[:(nx+1)*(nz+1)*NUM_VARS], tend[:nx*nz*NUM_VARS])
  #pragma acc parallel loop collapse(3) private(indt, indf1, indf2)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (i=0; i<nx; i++) {
...
void set_halo_values_x( double *state ) {
  ...
  #pragma acc data present(state[:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) copyout(sendbuf_r[:hs*nz*NUM_VARS], sendbuf_l[:hs*nz*NUM_VARS])
  ...
  //Fire off the sendsnx*nz*NUM_VARS
  ...
  #pragma acc data present(state[:(nx+2*hs)*(nz+2*hs)*NUM_VARS])  copyin(recvbuf_l[:hs*nz*NUM_VARS], recvbuf_r[:hs*nz*NUM_VARS])
  #pragma acc parallel loop collapse(3)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (s=0; s<hs; s++) {
  ...
  if (data_spec_int == DATA_SPEC_INJECTION) {
    if (myrank == 0) {
      #pragma acc data present(state[:(nx+2*hs)*(nz+2*hs)*NUM_VARS], hy_dens_cell[2:nz], hy_dens_theta_cell[2:nz])
...
void set_halo_values_z( double *state ) {
  ...
  // TODO: THREAD ME
  /////////////////////////////////////////////////
  #pragma acc data present(state[:(nx+2*hs)*(nz+2*hs)*NUM_VARS])
  #pragma acc parallel loop collapse(2) private(xloc, mnt_deriv)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (i=0; i<nx+2*hs; i++) {
      if (ll == ID_WMOM) {
...
void init( int *argc , char ***argv ) {
  ...
  #pragma acc enter data create(flux[:(nx+1)*(nz+1)*NUM_VARS])
  #pragma acc enter data create(tend[:nx*nz*NUM_VARS])
  ...
  #pragma acc enter data copyin(state[:(nx+2*hs)*(nz+2*hs)*NUM_VARS])
  #pragma acc enter data copyin(state_tmp[:(nx+2*hs)*(nz+2*hs)*NUM_VARS])
  #pragma acc enter data copyin(hy_dens_cell[:(nz+2*hs)])
  #pragma acc enter data copyin(hy_dens_theta_cell[:(nz+2*hs)])
  #pragma acc enter data copyin(hy_dens_int[:(nz+1)])
  #pragma acc enter data copyin(hy_dens_theta_int[:(nz+1)])
  #pragma acc enter data copyin(hy_pressure_int[:(nz+1)])
}
...
void output( double *state , double etime ) {
  ...
  if (etime == 0) {
    //Create the file
    ncwrap( ncmpi_create( MPI_COMM_WORLD , "new.nc" , NC_CLOBBER , MPI_INFO_NULL , &ncid ) , __LINE__ );
    ...
    //Open the file
    ncwrap( ncmpi_open( MPI_COMM_WORLD , "new.nc" , NC_WRITE , MPI_INFO_NULL , &ncid ) , __LINE__ );
...
void finalize() {
  int ierr;
  #pragma acc exit data delete(flux[:(nx+1)*(nz+1)*NUM_VARS])
  #pragma acc exit data delete(tend[:nx*nz*NUM_VARS])
  #pragma acc exit data delete(state[:(nx+2*hs)*(nz+2*hs)*NUM_VARS])
  #pragma acc exit data delete(state_tmp[:(nx+2*hs)*(nz+2*hs)*NUM_VARS])
  #pragma acc exit data delete(hy_dens_cell[:(nz+2*hs)])
  #pragma acc exit data delete(hy_dens_theta_cell[:(nz+2*hs)])
  #pragma acc exit data delete(hy_dens_int[:(nz+1)])
  #pragma acc exit data delete(hy_dens_theta_int[:(nz+1)])
  #pragma acc exit data delete(hy_pressure_int[:(nz+1)])
  free( state );
  free( state_tmp );
  free( flux );
  free( tend );
